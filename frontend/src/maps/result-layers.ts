...
const emptyCollection = (): ResultCollection => ({ type: 'FeatureCollection', features: [] });
...
const coordinates = ({ lat, lng }: Point): Position => [lng, lat];
const feature = (
  kind: string,
  geometry: Geometry,
  properties: Omit<ResultProperties, 'kind'> = {}
): Feature<Geometry, ResultProperties> => ({
  type: 'Feature',
  properties: { kind, ...properties },
  geometry
});

function unwrapLine(points: readonly Point[]) {
  const line: Position[] = [];
...
function drawMarker(
  ...
  const filterContext = context as CanvasRenderingContext2D & { filter?: string };
  if (typeof filterContext.filter === 'string') {
...
function markerSprites(accent: string): Promise<MarkerSprites> {
...
function addLayer(map: MapLibreMap, layer: LayerSpecification) {
...
export class ResultLayers {
  ...
  private data: ResultCollection = emptyCollection();
  private installRevision = 0;
  private dragging = false;

  constructor(
    map: MapLibreMap,
    onAnswerClick: (location: Location) => void,
    accent: string,
    dark = false
  ) {
    this.map = map;
  ...
  answerAt(point: PointLike | null): Location | null {
  ...
  updateCursor(point: PointLike | null = null) {
