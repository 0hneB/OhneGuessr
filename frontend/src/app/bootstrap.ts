...
async function setupMapSources() {
  const [{ setupMapMakingApp }, { setupLearnableMetaMapSource }] = await Promise.all([
    import('../features/map-making-app/setup.js'),
    import('../features/learnable-meta/setup.js')
  ]);
  setupMapMakingApp();
...
if (route.view === 'game' || challengeID) {
  ...
  if (partyID) {
    await import('./launcher/launcher.css');
    const { setupLocalPartyHost } = await import('../features/local-party/host-game.js');
    ...
    const { installCountryStreakGame } = await import(
      '../features/country-streak/game.svelte.js'
    );
  ...
  const { setupChallengeLauncher } = await import('../features/challenges/setup.js');
