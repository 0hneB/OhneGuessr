...
const SEARCH_RADII = [100_000, 300_000] as const;
...
const errorName = (error: unknown) =>
  cleanText((error as { name?: unknown } | null)?.name);

function safeURL(value: unknown, httpsOnly = false) {
  try {
...
function rankStations(value: unknown, limit: number) {
  ...
  for (const item of value) {
    ...
    if (!uuid || seen.has(uuid) || !name || !streamURL || !Number.isFinite(distance) || distance < 0
      || Number(source.hls) !== 0 || Number(source.lastcheckok) !== 1) continue;
    seen.add(uuid);
    stations.push({
      uuid,
      name,
...
async function requestStations(
  ...
  if (!response.ok) throw new Error('The radio directory is unavailable.');
  let body: unknown;
  try { body = await response.json(); }
  catch { throw new Error('The radio directory returned an invalid response.'); }
  return rankStations(body, limit);
}

export async function findNearestStation(
  position: { lat: number; lng: number },
  tryStation: (station: Station, signal: AbortSignal) => Promise<void>,
  signal: AbortSignal,
  ...
  for (const radius of SEARCH_RADII) {
    ...
    for (const station of candidates) {
      try {
        ...
        if (errorName(error) === 'AbortError' || errorName(error) === 'NotAllowedError') throw error;
...
function activate(api: OhneGuessrPluginAPI) {
  ...
  const setStatus = (message: string, kind: '' | 'loading' | 'error' = '', canRetry = false) => {
  ...
  const tryStation = (station: Station, signal: AbortSignal) => {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const timer = window.setTimeout(
        () => finish(new Error('The station took too long to start.')),
        STREAM_TIMEOUT
      );
      const cleanup = () => {
      ...
      try { void audio.play().catch(finish); }
  ...
  const tune = async (resetFailures = false) => {
    ...
    try {
      ...
      if (errorName(error) === 'AbortError' || id !== sequence || !active) return;
  ...
  website.addEventListener('click', () => {
    if (current?.homepage) void api.ui.openExternal(current.homepage);
  });
  volume.addEventListener('input', () => {
    ...
    try { localStorage.setItem(VOLUME_STORAGE, String(audio.volume)); } catch {}
  });
