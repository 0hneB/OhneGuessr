...
const CAMERA_LABELS: Record<NonNullable<OhneGuessrPanoramaDetails['cameraType']>, string> = {
  gen1: 'Gen 1', gen2: 'Gen 2', gen4: 'Gen 4', badcam: 'Bad cam', tripod: 'Tripod', trekker: 'Trekker'
};
const PANO_LABELS: Record<OhneGuessrPanoramaDetails['panoType'], string> = {
  official: 'Official', 'user-uploaded': 'User-uploaded', unknown: 'Unknown'
};
const cleanText = (value: unknown) => typeof value === 'string' ? value.trim() : '';

type CoverageMetadata = OhneGuessrPanoramaMetadata & Partial<OhneGuessrPanoramaDetails>;
type DetailRow = readonly [label: string, value: Node | string | null | undefined, className?: string];
...
function addressLabel(key: string) {
...
export function flagSources(countryCode: unknown) {
  const code = cleanText(countryCode).toLowerCase();
  if (!/^[a-z]{2}$/.test(code)) return null;
  return { src: `${FLAG_ENDPOINT}/${code}.svg` };
}

export function streetViewURL(metadata?: StreetViewMetadata | null) {
  const panoId = cleanText(metadata?.panoId);
  const lat = metadata?.position?.lat;
  const lng = metadata?.position?.lng;
  if (!panoId || !Number.isFinite(lat as number) || !Number.isFinite(lng as number)) return '';
...
export function locationComponents(value: unknown): LocationComponent[] {
...
function detailList(rows: readonly DetailRow[]) {
...
const number = (value: unknown, digits = 1) => typeof value === 'number' && Number.isFinite(value)
...
function countryNode(location: OhneGuessrLocationDetails) {
...
function render(
  api: OhneGuessrPluginAPI,
  content: HTMLDivElement,
  metadata: CoverageMetadata,
  location: OhneGuessrLocationDetails | null,
  locationState: LocationState = {}
) {
  const root = element('div', 'coverage-info');
  const locationChildren: Node[] = [];
  if (location) {
    ...
    if (rows.length) locationChildren.push(detailList(rows));
  } else {
    const status = element('div', `coverage-info-status${locationState.error ? ' error' : ''}`);
    if (locationState.loading) status.append(element('span', 'coverage-info-spinner'));
  ...
  root.append(section('Street View coverage', panoramaLink, detailList([
    ...
    ['Camera type', CAMERA_LABELS[metadata.cameraType as NonNullable<OhneGuessrPanoramaDetails['cameraType']>] || ''],
    ['Panorama type', PANO_LABELS[metadata.panoType as OhneGuessrPanoramaDetails['panoType']] || ''],
