...
async function responseJSON(response, origin) {
    if (!response.ok) {
        if (response.status === 401 || response.status === 403)
            throw new Error(accessError(origin));
        if (response.status === 429)
            throw new Error('PlantNet API quota reached');
...
export async function identifyPlants(apiKey, image, signal, origin = globalThis.location?.origin || '') {
...
function activate(api) {
  ...
  `;
    ...
    const renderStatus = (message, error = false) => {
        ...
        if (!error)
            body.appendChild(element('div', 'plantnet-spinner'));
    ...
    const renderResults = ({ results }) => {
        ...
        if (results.length) {
            ...
            for (const result of results) {
                ...
                if (result.family)
                    meta.appendChild(element('span', '', `Family: ${result.family}`));
