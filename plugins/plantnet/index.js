...
async function responseJSON(response) {
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) throw new Error(SETTINGS_ERROR);
...
function activate(api) {
  ...
  style.textContent = `
    ...
    .plantnet-status { display:grid; min-height:160px; place-items:center; color:#c8ced8; text-align:center }
