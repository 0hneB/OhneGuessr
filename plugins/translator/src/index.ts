...
type TargetLanguageCode = typeof TARGET_LANGUAGES[number][0];
type RequestKind = '' | 'error' | 'loading';
...
const languageCodes = new Map(
  TARGET_LANGUAGES.map(([code]) => [code.toLowerCase(), code] as const)
);
...
export function normalizeTargetLanguage(value: unknown): TargetLanguageCode {
...
export function parseOCRResponse(value: unknown) {
  const source = value && typeof value === 'object' ? value as Record<string, unknown> : {};
  const results = Array.isArray(source.ParsedResults) ? source.ParsedResults : [];
  const detected = results
    .map((result) => cleanText((result as Record<string, unknown> | null)?.ParsedText))
    .filter(Boolean)
    .join('\n\n');
  ...
  const message = [
    ...
    ...results.flatMap((result) =>
      errorMessages((result as Record<string, unknown> | null)?.ErrorMessage))
  ][0];
  if (source.IsErroredOnProcessing || message
    || source.OCRExitCode && source.OCRExitCode !== 1) {
    throw new Error(message || 'OCR.space could not read this image.');
  }
  return '';
}

export function splitTranslationText(value: unknown, maximum = 500) {
...
export function parseTranslationResponse(value: unknown) {
  const source = value && typeof value === 'object' ? value as TranslationResponse : {};
...
export async function recognizeText(apiKey: string, image: Blob, signal?: AbortSignal) {
...
export async function translateText(value: unknown, target: unknown, signal?: AbortSignal) {
...
function activate(api: OhneGuessrPluginAPI) {
  ...
  style.textContent = `
    .translator { display:grid; gap:11px; color:var(--launcher-text,#e8ebf0) }
    .translator-toolbar { display:flex; align-items:center; justify-content:space-between; gap:12px;
      padding:9px 10px; background:var(--launcher-main,#202020);
      border:1px solid var(--launcher-border,rgba(255,255,255,.09)); border-radius:6px }
    .translator-target { width:100%; flex-direction:row; align-items:center; gap:9px; margin:0;
      color:var(--launcher-text-muted,#b8c0cb); font-size:13px }
    .translator-select { flex:1; width:auto; min-width:0 }
    .translator-target select { min-width:0; color:var(--launcher-text,#fff); background:var(--launcher-field,#222);
      border:1px solid var(--launcher-field-border,rgba(255,255,255,.09)) }
    ...
    .translator-status { display:flex; min-height:54px; align-items:center; justify-content:center; gap:9px;
      padding:12px; color:var(--launcher-text-muted,#c6cdd7); background:var(--launcher-main,#202020);
    ...
    .translator-status.error, .translator-text.error { color:var(--launcher-error,#fca5a5) }
    .translator-spinner { width:15px; height:15px; flex:none;
      border:2px solid var(--launcher-border,rgba(255,255,255,.18));
    ...
    .translator-card { min-width:0; overflow:hidden; background:var(--launcher-main,#202020);
      border:1px solid var(--launcher-border,rgba(255,255,255,.09)); border-radius:6px }
    .translator-card h3 { margin:0; padding:8px 10px; color:var(--launcher-text,#fff);
      border-bottom:1px solid var(--launcher-border,rgba(255,255,255,.09));
      font-size:13px; letter-spacing:.03em }
    .translator-text { min-height:118px; max-height:320px; margin:0; padding:11px; overflow:auto;
      color:var(--launcher-text-soft,#edf0f5); font:14px/1.5 system-ui,sans-serif;
  ...
  let sequence = 0;
  let originalText = '';
  let button: HudButton | null = null;
  ...
  const setStatus = (message = '', kind: RequestKind = '') => {
  ...
  const active = (id: number) => id === sequence;
  const finish = (controller: AbortController, id: number) => {
  ...
  const renderTranslation = async (controller: AbortController, id: number) => {
    setTranslationTitle();
    ...
    try {
      ...
      if ((error as { name?: unknown } | null)?.name === 'AbortError' || !active(id)) return;
  ...
  const scan = async () => {
    ...
    try {
      ...
      if ((error as { name?: unknown } | null)?.name !== 'AbortError' && active(id)) {
