...
export function splitTranslationText(value, maximum = 500) {
    if (!Number.isInteger(maximum) || maximum < 4)
        throw new Error('Invalid translation chunk size');
...
export function parseTranslationResponse(value) {
    const source = value && typeof value === 'object' ? value : {};
    if (source.quotaFinished)
        throw new Error('The MyMemory daily translation quota has been reached.');
...
export async function recognizeText(apiKey, image, signal) {
    ...
    if (response.status === 429)
        throw new Error('The OCR.space request limit has been reached.');
    if (!response.ok)
        throw new Error('Text detection failed. Please try again.');
    let body;
    try {
        body = await response.json();
    }
    catch {
        throw new Error('OCR.space returned an invalid response.');
    }
...
export async function translateText(value, target, signal) {
    ...
    for (const chunk of chunks) {
        ...
        if (response.status === 429)
            throw new Error('The MyMemory request limit has been reached.');
        if (!response.ok)
        ...
        try {
            ...
            throw new Error('MyMemory returned an invalid response.');
        }
...
function jpeg(canvas, quality) {
    return new Promise((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Could not prepare the captured image.')), 'image/jpeg', quality));
}
async function prepareImage(source) {
    ...
    try {
        ...
        if (!context)
            throw new Error('Could not prepare the captured image.');
...
function activate(api) {
  ...
  `;
    ...
    let savedTarget = '';
    try {
        savedTarget = localStorage.getItem(TARGET_STORAGE) || '';
    }
    catch { }
    ...
    const setStatus = (message = '', kind = '') => {
        ...
        if (kind === 'loading')
            status.append(element('span', 'translator-spinner'));
        if (message)
    ...
    const finish = (controller, id) => {
        if (active(id) && request === controller)
            request = null;
    };
    ...
    const renderTranslation = async (controller, id) => {
        ...
        try {
            ...
            const detected = TARGET_LANGUAGES.find(([code]) => code.toLowerCase() === translation.detectedLanguage.toLowerCase())?.[1];
