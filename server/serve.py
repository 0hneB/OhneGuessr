...
Serves dist/ over http:// and adds a write API so uploaded maps are saved under
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        ...
        if first == "server" or first.startswith("."):
            return os.path.join(DIST_DIR, ".not-found")
        try:
...
def main():
    ...
    if not os.path.isfile(os.path.join(DIST_DIR, "index.html")):
        raise SystemExit("Missing dist/index.html. Download a complete release or run npm run build.")
